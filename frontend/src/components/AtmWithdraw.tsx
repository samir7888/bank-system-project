...
const ATMInterface = () => {
  ...
  const [withdrawnAmount, setWithdrawnAmount] = useState<Record<
    string,
    number
  > | null>(null);
  ...
  const handleWithdraw = async () => {
    ...
    try {
      const data = await atmWithdraw({ amount: Number(amount) });
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex items-center justify-center">
      <div className=" rounded-xl shadow-2xl overflow-hidden">
            ...
            <div className="bg-blue-900 h-40 rounded-md p-4 mb-4 flex flex-col justify-between shadow-inner">
              ...
              <div className="flex flex-col items-center justify-center flex-grow">
                {dispensing ? (
                  ...
                  <div className="text-center">
                    <div className="text-green-300 text-2xl font-bold mb-2">
                      Success!
                    </div>
              ...
              {error && (
                <div className="bg-red-900 text-white p-2 rounded text-center">
                  {error}
                </div>
              )}
            </div>
        ...
        {/* Cash Dispenser */}
        <div
          className="bg-gray-700 p-4 relative overflow-hidden"
          style={{ minHeight: Math.max(120, totalNotes * 30) + "px" }}
        >
              ...
              {withdrawnAmount &&
                Object.entries(withdrawnAmount).map(([denom, count]) =>
                  Array.from({ length: count }).map((_, idx) => (
                    ...
                    >
                      <img
                        src={noteImages[denom]}
                        alt={`${denom} note`}
                        className="w-40 h-12 object-cover rounded-sm shadow-md"
                      />
                    </div>
                  ))
                )}
