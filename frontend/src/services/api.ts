...
import {
  ...
  TransferFormData,
  UserWithBalance,
  EmergencyCreditStatusResponse,
  MaintenanceStatus,
  EmergencyCreditClaimResponse,
  RegisterFormData,
  UserResponse,
...
export const getTransactionHistory = async (page: number, limit: number) => {
...
export const getEmergencyWalletStatus = async () => {
  ...
  const response = await api.get<EmergencyCreditStatusResponse>(
    `/emergency-credit/status/${id}`
  );
  return response.data;
};
...
export const ClaimEmergencyWallet = async () => {
  const response = await api.post<EmergencyCreditClaimResponse>(
    "/emergency-credit/claim"
  );
  return response.data;
};

export const checkStatusOfMaintenance = async () => {
  const response = await api.get<MaintenanceStatus>(
    "/emergency-credit/maintenance-alert/status"
  );
  return response.data;
};

export const settleOfflineTransfer = async ({
  receiver,
  amount,
}: {
  receiver: string;
...
export const isMaintenanceActive = async () => {
  const response = await api.get<IisMaintenanceActive>(
    "/emergency-credit/maintenance/active"
  );
  return response.data;
};
